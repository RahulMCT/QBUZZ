...
--
-- Description : Extensible Framework JavaScript OUWOCF - Work Order Control
--
-- Product     : EAM             Release : 11.7
--
-- Author      : Deepu Kukreja
--
-- CHANGE HISTORY
--
-- 0001 21-09-2022 - MCT - Creation
-- 0002 09-11-2022 - MCT - Commented the Generate PO Button
-- 0003 11-11-2022 - MCT - Protected Some fields based on Status.
-- -------------------------------------------------------------------------------
 */
            ...
            '[extensibleFramework] [tabName=HDR]': {
                ...
                afterrender: function (a, b) {
                    ...
                    if (EAM.getApplication().designerMode === false && a.userFunction === vFunCode && a.tabName === vTabName) {
                        ...
                        var vUpperFld = formPanel.getFld("wspf_10_ctl_status");
...
function setFldWith(formPanel) {
    try {
        ...
        var vDateGenFld = formPanel.getFld("wspf_10_ctl_wccreated");
        ...
        debug && console.log('Error in setFldWith : ' + e);
    }
};
                ...
                listeners: {
                    'select': function (combo, records, value) {
                        ...
                        if (vPageMode === "view") {
                            ...
                            var vOldRStatus = combo.originalValue;
                            var vNewRStatus = combo.value;
                        }
                        var vStatusFld = formPanel.getFld("wspf_10_ctl_status");
                        if (EAM.getApplication().designerMode === false) {
...
function handleStatusChange(formPanel, changeMode) {
    try {
        ...
        if (changeMode === "DELAYED") {
		    ...
		    /*if (userGroup === "VLOOT-MAN" && vStatus === "WCC") {
                if (Ext.isEmpty(formPanel.getFldValue("wspf_10_ctl_po"))) {
                    vgenpoButton.setDisabled(false);
                }
            }*/
            if ((userGroup === "SUPPLIER" && vStatus === "SC") || (vStatus === "WCC")) {
                if (vStatusDropDown !== "") {
...
function insertRecord(formPanel) {
    try {
        ...
        var requestData = {
            ...
            'wspf_10_prs_param1': formPanel.getFldValue("wspf_10_ctl_org"),
            'wspf_10_prs_screen': 'OUWOCF',
            'wspf_10_prs_param2': formPanel.getFldValue("wspf_10_ctl_seq"),
