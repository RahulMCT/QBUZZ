...
-- CHANGE HISTORY
--
-- 0001 21-12-2022  - MCT - SPC301  - Creation
-- -------------------------------------------------------------------------------
...
BEGIN
  ...
  END IF;
  --Labor cost
  SELECT MIN(ddf_fieldid) INTO nfieldid33 FROM r5ddfield WHERE ddf_tablename = 'R5CASEMANAGEMENT' AND UPPER(ddf_sourcename) = 'NVL((SELECT EVO_OWNLABOR FROM R5EVENTCOST WHERE EVO_EVENT = CSM_WORKORDER),0)';
  IF nfieldid33 IS NULL THEN
    r5o7.o7maxseq(nfieldid33,'DDF','1',chk );
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_updatecount,ddf_tablename) VALUES (nfieldid33,'NVL((SELECT EVO_OWNLABOR FROM R5EVENTCOST WHERE EVO_EVENT = CSM_WORKORDER),0)','VARCHAR',0,'R5CASEMANAGEMENT');
  END IF;
  --Hired Labor
  ...
  IF nfieldid35 IS NULL THEN
    ...
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_updatecount,ddf_tablename) VALUES (nfieldid35,'NVL((SELECT SUM(COS_COST) FROM U5DMCOST WHERE COS_CASE = CSM_CODE AND COS_COSTCODE= 4406),0)','VARCHAR',0,'R5CASEMANAGEMENT');
  END IF;
  --CSM_COMPLETEDDATE
  ...
  END IF;
  --Kostentotaal
  SELECT MIN(ddf_fieldid) INTO nfieldid37 FROM r5ddfield WHERE ddf_tablename = 'R5CASEMANAGEMENT' AND UPPER(ddf_sourcename) = '(O7COST.MATERIAL2(CSM_WORKORDER)+O7COST.LABOUR2(CSM_WORKORDER)+NVL((SELECT EVO_OWNLABOR FROM R5EVENTCOST WHERE EVO_EVENT = CSM_WORKORDER),0))';
  IF nfieldid37 IS NULL THEN
    r5o7.o7maxseq(nfieldid37,'DDF','1',chk );
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_updatecount,ddf_tablename) VALUES (nfieldid37,'(o7cost.material2(csm_workorder)+o7cost.labour2(csm_workorder)+NVL((select evo_ownlabor from r5eventcost where evo_event = csm_workorder),0))','VARCHAR',0,'R5CASEMANAGEMENT');
  END IF;
  --CSM_UDFNUM04
  ...
  IF nfieldid38 IS NULL THEN
    ...
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_updatecount,ddf_tablename) VALUES (nfieldid38,'NVL((SELECT SUM(cos_cost) FROM u5dmcost WHERE cos_case = csm_code AND cos_costcode= 4407),0)','VARCHAR',0,'R5CASEMANAGEMENT');
  END IF;
  --Schadekosten Qbuzz
  SELECT MIN(ddf_fieldid) INTO nfieldid39 FROM r5ddfield WHERE ddf_tablename = 'R5CASEMANAGEMENT' AND UPPER(ddf_sourcename) = 'NVL(CSM_UDFNUM01,0)+ NVL(CSM_UDFNUM02,0)+ NVL(CSM_UDFNUM03,0)+NVL((SELECT SUM(COS_COST) FROM U5DMCOST WHERE COS_CASE = CSM_CODE AND COS_COSTCODE= 4407),0)+NVL((O7COST.MATERIAL2(CSM_WORKORDER)+O7COST.LABOUR2(CSM_WORKORDER)+NVL((SELECT evo_ownlabor FROM R5EVENTCOST WHERE EVO_EVENT = CSM_WORKORDER),0)),0)+NVL((SELECT SUM(COS_COST) FROM U5DMCOST WHERE COS_CASE = CSM_CODE AND COS_COSTCODE= 4406),0)';
  IF nfieldid39 IS NULL THEN
    r5o7.o7maxseq(nfieldid39,'DDF','1',chk );
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_updatecount,ddf_tablename) VALUES (nfieldid39,'NVL(CSM_UDFNUM01,0)+ NVL(CSM_UDFNUM02,0)+ NVL(CSM_UDFNUM03,0)+NVL((SELECT SUM(COS_COST) FROM U5DMCOST WHERE COS_CASE = CSM_CODE AND COS_COSTCODE= 4407),0)+NVL((O7COST.MATERIAL2(CSM_WORKORDER)+O7COST.LABOUR2(CSM_WORKORDER)+NVL((SELECT evo_ownlabor FROM R5EVENTCOST WHERE EVO_EVENT = CSM_WORKORDER),0)),0)+NVL((SELECT SUM(COS_COST) FROM U5DMCOST WHERE COS_CASE = CSM_CODE AND COS_COSTCODE= 4406),0)','VARCHAR',0,'R5CASEMANAGEMENT');
  END IF;
  --set query
  sQuery :=
 'FROM r5casemanagement
  ...
  INSERT INTO r5boilertexts(bot_function,bot_number,bot_length,bot_xpos,bot_ypos,bot_lang,bot_text,bot_dest,bot_page,bot_fld1, bot_fld2, bot_prtp, bot_lvcr, bot_adlen, bot_pool,bot_changed, bot_display, bot_updatecount,bot_cloned)
  SELECT sFunction,36,100,0,0,lin_code, 'Betaald ER'              ,'F' ,1 ,'CSMEXTCOST'    ,NULL ,NULL ,NULL ,NULL ,'10000' ,'+' ,'-' ,0 ,NULL
...
--Merge into Another trigger.
DELETE FROM r5flexsql WHERE flx_table = 'U5DMCOST';
DROP TRIGGER COMP_INSUPD_U5ACS_QBZ;
...
BEGIN
  IF :NEW.csm_workorder IS NOT NULL THEN
    ...
    FROM r5events
    WHERE evt_code = :NEW.csm_workorder
    AND evt_status = 'C';
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
        RETURN;
    ...
    IF countit = 0 THEN
      ...
      cnt := cnt+10;
      INSERT INTO u5dmcost (cos_case, cos_line, cos_costdesc, cos_costcode, cos_costcodedesc, cos_cost, cos_totalcost, createdby, created, cos_date,cos_wo)
                     VALUES(:NEW.csm_code, cnt, :NEW.csm_desc, '4405', 'betaalde schade reparatie algemeen', Ccost, NULL, cuser, SYSDATE, TRUNC(SYSDATE),'+');
    END IF;--countit
  END IF;
END PREINS_CSM_QBZ;
/
...
PROMPT ...Creating trigger PREUPD_CSM_QBZ
PROMPT

CREATE OR REPLACE TRIGGER PREUPD_CSM_QBZ
BEFORE UPDATE ON r5casemanagement
FOR EACH ROW
DECLARE
  countit    NUMBER;
  cnt        NUMBER;
  ...
  cuser      r5users.usr_code%type;
BEGIN
  IF :NEW.csm_workorder IS NOT NULL
    AND :OLD.csm_workorder IS NULL THEN
    BEGIN
    SELECT (o7cost.tools2(evt_code)+o7cost.material2(evt_code)+o7cost.labour2(evt_code)) INTO Ccost
    FROM r5events
    WHERE evt_code = :NEW.csm_workorder
    ...
    IF countit = 0 THEN
      ...
      INSERT INTO u5dmcost (cos_case, cos_line, cos_costdesc, cos_costcode, cos_costcodedesc, cos_cost, cos_totalcost, createdby, created, cos_date,cos_wo)
                     VALUES(:NEW.csm_code, cnt, :NEW.csm_desc, '4405', 'betaalde schade reparatie algemeen', Ccost, NULL, cuser, SYSDATE, TRUNC(SYSDATE),'+');
    END IF;--countit
  END IF;--csm_workorder
END PREUPD_CSM_QBZ;
/
SHOW ERRORS TRIGGER PREUPD_CSM_QBZ
PROMPT

SET VERIFY ON
SET SERVEROUTPUT OFF

EXEC QBZ_O7CUSTOM( 'SPC-301.sql', '21-12-2022', 'QBZ-EAM 11.7', 'Changes- Damage delivery');

COMMIT;

PROMPT ...END of SPC-301.log
