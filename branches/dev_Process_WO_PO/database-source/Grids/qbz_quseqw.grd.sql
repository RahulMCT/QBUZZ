-- -------------------------------------------------------------------------------
--
-- Filename    : qbz_quseqw.grd.sql
--
-- Description : Grid to get sequence of U5WOCONT
--
...
PROMPT Begin of qbz_quseqw.grd.sql
PROMPT

PROMPT ...Creating grid QUSEQW
PROMPT

DECLARE
  sFunction      VARCHAR(6) := 'QUSEQW';
  sGridid        VARCHAR(30);
...
BEGIN
  ...
  VALUES(sFunction ,'Sequence for U5WOCONT'  ,'WEBD' ,'X'  ,'R5MENU'
        ,NULL      ,NULL            ,NULL   ,'?'    ,'YY'
        ,'D'       ,'-'             ,'-'    ,'+'    ,'*'
        ,'+'       ,'-'             ,'+'    ,NULL   ,'S');
  --create ddfield
  SELECT MIN(ddf_fieldid) INTO ddfield1 FROM r5ddfield WHERE UPPER(ddf_sourcename) = UPPER('nvl((SELECT max(to_number(ctl_seq)) + 1 FROM U5WOCONT), 1000)');
  IF ddfield1 IS NULL THEN
    r5o7.o7maxseq(ddfield1,'DDF','1',chk );
    INSERT INTO r5ddfield(ddf_fieldid,ddf_sourcename,ddf_datatype,ddf_tablename) VALUES (ddfield1,'nvl((SELECT max(to_number(ctl_seq)) + 1 FROM U5WOCONT), 1000)','MIXVARCHAR',NULL);
  END IF;
  ...
  INSERT INTO r5boilertexts (bot_function,bot_number,bot_length,bot_xpos,bot_ypos,bot_lang,bot_text,bot_dest,bot_page,bot_fld1,bot_pool,bot_changed,bot_display,bot_cloned) VALUES(sFunction,2,100,0,0,'EN','Sequence for U5WOCONT','F',1,'UDG_HDRLABEL','10000','+',NULL,'-');
...
PROMPT ...Created grid QUSEQW

PROMPT End of qbz_quseqw.grd.sql
PROMPT
