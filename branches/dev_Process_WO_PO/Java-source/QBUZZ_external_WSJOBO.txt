...
-- 0025 04-03-2024 - SPC-640 Check Apply for all price Fld.
-- 0026 03-06-2024 - SPC-698 Set layout for Part and Part Description, and populate Part Description.
-- -------------------------------------------------------------------------------
 */
                ...
                before_eam_beforesaverecord: function (a, b, c) {
                    if (a.userFunction === "WSJOBO" && a.tabName === "U1") {
                        ...
                        if (vParts && vCatalogPrice < 0) { // Changed <== to <=
                            var VuserLang = EAM.AppData.getInstallParams().get("language");
                            if (VuserLang === "NL") { /*Error Message Change SPC-630/640*/
                        ...
                        var vAmount = formPanel.getFldValue("wspf_10_ou_amt");
                        if (vAmount < 0) { // Check if the value is zero or less
                            var vUserLang = EAM.AppData.getInstallParams().get("language");
                            if (vUserLang === "NL") {
                                EAM.Messaging.showError('Totale kosten Moet groter zijn dan 0.');
                            } else {
                                EAM.Messaging.showError('Amount must be greater than 0.');
                            }
                            return false;
                        }
